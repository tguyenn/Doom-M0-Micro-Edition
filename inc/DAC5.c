...
#include "../inc/LaunchPad.h" // for port indices
...
// I.e., PortB has already been reset and activated (do not reset PortB here again)
    GPIOB->DOE31_0 |= PB0INDEX | PB1INDEX | PB2INDEX | PB3INDEX | PB4INDEX;
...
void DAC5_Out(uint32_t data){
    // make each bit correspond to its respective output put ie PB0 = bit 0
    GPIOB->DOUT31_0 = (~0x1F & GPIOB->DOUT31_0) | data;
}
